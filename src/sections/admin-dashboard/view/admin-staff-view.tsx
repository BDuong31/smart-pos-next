...
import { getListStaff, } from '@/apis/user';
...
export default function StaffView() {
  ...
  const fecherUsers = async () => {
    try {
      setLoading(true);
      const response = await getListStaff({}, currentPage, ITEMS_PER_PAGE);
        ...
        <div className="card-body h-[65vh]">
          <div className="overflow-x-auto">
            <table className="table">
              ...
              <tbody>
                {usersList.map((user) => (
                  <tr key={user.id} className="hover" onClick={() => router.push(`/staff/${user.id}`)}>
                    <th><input type="checkbox" className="checkbox checkbox-sm" /></th>
                      <td>
                      ...
                      <td>
                        <Badge
                          text={user.role ==='admin' ? 'Quản lý' : user.role === 'staff' ? 'Phục vụ' : user.role === 'kitchen' ? 'Bếp' : 'Khách hàng'} 
                          type={user.role === 'admin' ? 'info' : user.role === 'staff' ? 'success' : user.role === 'kitchen' ? 'warning' : 'neutral'} 
                        />
                      </td>
                      <td>
                        <Badge 
